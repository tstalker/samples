#ifdef BASE_HPP
#error Redefined header base.hpp
#endif
...
template <typename T>
auto ier::Base<T>::PrintBraces(std::ostream& s) const
-> std::ostream&
{
	s << '{';
...
template <typename T>
decltype(auto)
ier::operator << (std::ostream& s, const Base<T> &x)
{
...
template <typename T>
decltype(auto)
ier::operator << (std::ostream& s, const std::unique_ptr<T> &p)
{
	return s << *p;
}
